# Step 14 — European Non-Household Electricity & Gas Price Benchmark
...
- later financial opportunity modelling;
...
The package-local `scripts/download_eurostat.py`, `scripts/validate_bronze.py`,
and `scripts/bronze_to_silver.py` form the superseded metadata-only route. They
are retained for history but are not selected by
