...
mandatory fail-fast validations. The runner correctly stopped when SQL
...
- Electricity and cost totals/order matched the governed energy snapshot and
  mandatory energy/utilities/cost gates.
...
- Failures: 46,670; downtime:
  ...
  0.282998226 h.
- Monthly reliability trend: 720 rows across
  24 months; SQL 740 was not run.
- Forecast: 720 rows; energy anomaly:
  32,850 rows; reliability bridge:
  720 rows.
- Loader audit hashes matched the accepted energy-anomaly and forecasting snapshots.

## SQL 623 finding

The canceled SQL 623 execution repeatedly expanded the nested accepted Laney
p-prime view and its section-4 aggregation ran for more than 40 minutes. The
validation materializes the unchanged accepted view once into a session-local
