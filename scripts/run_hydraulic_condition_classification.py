...
def repo_root() -> Path:
    ...
    for c in [here, *here.parents]:
        if (c / "sources" / "hydraulic-condition-monitoring").exists():
            return c
    raise FileNotFoundError("Could not locate sources/hydraulic-condition-monitoring.")
...
def evaluate_target(
    ...
    pred_out.to_csv(
        ...
        out_dir / f"accepted_{target}_feature_importance.csv",
        index=False,
    )

    joblib.dump(
        model,
        model_dir / f"accepted_{target}_extra_trees.joblib",
    )
...
def main():
    ...
    silver = repo / "sources" / "hydraulic-condition-monitoring" / "silver" / "reliability"
    ...
    if not feature_path.exists():
        raise FileNotFoundError(
            ...
            "Run hydraulic feature preparation once first to create cycle features."
        )

    print("=== Hydraulic condition classification ===")
    ...
    summary.to_csv(
        out_dir / "accepted_condition_model_summary.csv",
        index=False,
    )
    ...
    report_path = (
        report_dir
        / "hydraulic_condition_classification_metrics.json"
    )
    ...
    print("=== hydraulic condition classification complete ===")
