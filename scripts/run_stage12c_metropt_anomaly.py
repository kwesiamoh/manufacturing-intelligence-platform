...
# ---------------------------------------------------------------------------
# Stage 12C — MetroPT-3 causal anomaly detection
#
# Design:
#   raw cadence           : ~10 s
...
def locate_metropt_silver(repo: Path) -> Path:
    expected = repo / "sources" / "step02-telemetry" / "data" / "silver" / "telemetry" / "metropt3"
    if expected.exists():
        return expected
...
def main() -> None:
    ...
    print("=== Stage 12C MetroPT anomaly detection ===")
    ...
    with open(report_dir / "stage12c_telemetry_anomaly_metrics.json", "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)

    with open(report_dir / "stage12c_feature_columns.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(feature_cols))
    ...
    print(f"  {report_dir.relative_to(repo) / 'stage12c_telemetry_anomaly_metrics.json'}")
