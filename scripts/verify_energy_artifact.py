...
MANIFEST_PATH = REPO_ROOT / "config" / "energy_artifact_manifest.json"
...
class VerificationError(ValueError):
    """Raised when the governed energy and utilities snapshot no longer matches."""
...
def repository_path(relative_path: str) -> Path:
    ...
    if not path.is_file():
        raise VerificationError(f"Required energy and utilities artifact is missing: {path}")
    return path
...
def output_record(manifest: dict, suffix: str) -> dict:
    ...
    if len(matches) != 1:
        raise VerificationError(
            f"Expected exactly one energy and utilities manifest output ending in {suffix}"
        )
    return matches[0]
...
def validate() -> None:
    ...
    if manifest.get("release") != "v1.0.0":
        raise VerificationError("Unexpected energy artifact manifest release")

    generator = manifest["generator"]
    require_hash(
        repository_path(generator["path"]),
        generator["sha256"],
        "energy and utilities generator",
    )
    for input_record in manifest["inputs"]:
        require_hash(
            repository_path(input_record["path"]),
            input_record["sha256"],
            f"energy and utilities input {input_record['path']}",
        )
    for output in manifest["outputs"]:
        require_hash(
            repository_path(output["path"]),
            output["sha256"],
            f"energy and utilities output {output['path']}",
        )
    ...
    if list(line.columns) != LINE_COLUMNS:
        raise VerificationError("energy and utilities line-energy schema/order mismatch")
    if list(site.columns) != SITE_COLUMNS:
        raise VerificationError("energy and utilities site-energy schema/order mismatch")
    ...
    print("PASS: governed energy artifact verified")
...
def main() -> int:
    try:
        ...
        print(f"FAIL: governed energy artifact verification failed: {exc}", file=sys.stderr)
        return 1
