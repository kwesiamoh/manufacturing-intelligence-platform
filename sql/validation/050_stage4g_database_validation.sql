...
\echo '=== Stage 4G SQL validation complete ==='

\echo '=== Mandatory Stage 4G integrity gate ==='
DO $validation$
DECLARE
...
BEGIN
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'Stage 4G lineage gate found % orphan rows', bad_count;
    END IF;
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'Stage 4G uniqueness gate found % duplicate source-record groups', bad_count;
    END IF;
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'Stage 4G completeness gate found % rows with required nulls', bad_count;
    END IF;
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'Stage 4G business-rule gate found % invalid rows', bad_count;
    END IF;
END
$validation$;
