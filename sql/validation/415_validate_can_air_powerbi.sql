...
-- CAN compressed-air targeted propagation gate.
...
\echo '=== CAN compressed-air can-line compressed-air propagation ==='
SELECT
    layer_name,
    row_count,
    null_air_rows,
...
\echo '=== Mandatory CAN compressed-air propagation gate ==='
DO $validation$
DECLARE
    layer_record record;
...
BEGIN
    ...
    LOOP
        IF layer_record.row_count <> 6579
           ...
           OR ABS(layer_record.compressed_air_nm3 - 12805152.250) > 0.001 THEN
            RAISE EXCEPTION
                'CAN compressed-air failed at %: rows %, nulls %, wrong intensity %, total %',
    ...
    IF formula_bad_rows <> 0 THEN
        RAISE EXCEPTION
            'CAN compressed-air found % can-line rows that do not satisfy the generic utility formula',
            formula_bad_rows;
    END IF;
END
$validation$;
