...
\echo ''
\echo '=== reliability-trend validation complete ==='

\echo '=== Mandatory reliability-trend gate ==='
DO $validation$
DECLARE
    bad_count bigint;
...
BEGIN
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'reliability-trend found % failure events without exactly one source-qualified production parent', bad_count;
    END IF;
    ...
    IF (SELECT MIN(month_start) FROM analytics.vw_monthly_reliability_trend) <> DATE '2024-01-01'
       ...
       OR (SELECT COUNT(*) FROM analytics.vw_monthly_reliability_trend) <> 720 THEN
        RAISE EXCEPTION 'reliability-trend monthly trend coverage does not match 2024-01 through 2025-12';
    END IF;
    ...
    IF COALESCE((SELECT SUM(corrective_failure_count)
       ...
       <> expected_failure_events THEN
        RAISE EXCEPTION 'reliability-trend trends do not reconcile to canonical failure-event grain';
    END IF;
    IF ABS(COALESCE((SELECT SUM(corrective_downtime_hours)
       ...
       OR ABS(COALESCE((SELECT SUM(corrective_downtime_hours)
        ...
        RAISE EXCEPTION 'reliability-trend trends multiplied or omitted corrective downtime';
    END IF;
    IF ABS(COALESCE((SELECT SUM(mttr_hours * corrective_failure_count)
       ...
       OR ABS(COALESCE((SELECT SUM(mttr_hours * corrective_failure_count)
        ...
        RAISE EXCEPTION 'reliability-trend MTTR does not reconcile to work-order repair duration at failure-event grain';
    END IF;
    IF expected_failure_events <> 46670 THEN
        RAISE EXCEPTION 'reliability-trend accepted source snapshot changed from 46,670 corrective failure events';
    END IF;

    -- Exact dimensional/month reconciliation enforces reliability-trend production
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'reliability-trend found % site-line-month attribution mismatches', bad_count;
    END IF;
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'reliability-trend KPI sanity gate found % invalid monthly rows', bad_count;
    END IF;
END
$validation$;
