...
\echo '=== production benchmark row counts ==='
...
\echo '=== Mandatory production benchmark gate ==='
DO $validation$
DECLARE
    bad_count bigint;
BEGIN
    IF (SELECT COUNT(*) FROM vw_line_performance_summary) <> 30
       ...
       OR NOT EXISTS (SELECT 1 FROM vw_changeover_summary) THEN
        RAISE EXCEPTION 'production benchmark benchmark views have incomplete canonical coverage';
    END IF;
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'production benchmark KPI gate found % invalid line rows', bad_count;
    END IF;
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'production benchmark Pareto gate found % sites that do not reconcile to 1.0', bad_count;
    END IF;
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'production benchmark changeover gate found % invalid rows', bad_count;
    END IF;

    IF EXISTS (SELECT 1 FROM vw_site_production_benchmark WHERE line_count <> 5) THEN
        RAISE EXCEPTION 'production benchmark site benchmark does not contain five lines per site';
    END IF;
END
$validation$;
