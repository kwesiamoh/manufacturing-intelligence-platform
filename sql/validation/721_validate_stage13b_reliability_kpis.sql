...
\echo ''
\echo '=== Stage 13B validation complete ==='

\echo '=== Mandatory Stage 13B gate ==='
DO $validation$
DECLARE
    bad_count bigint;
...
BEGIN
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'Stage 13B found % corrective maintenance rows without exactly one source-qualified unplanned downtime parent', bad_count;
    END IF;
    ...
    IF (SELECT COUNT(*) FROM analytics.vw_corrective_maintenance_downtime_link)
       ...
       <> expected_work_orders THEN
        RAISE EXCEPTION 'Stage 13B work-order-grain lineage view does not reconcile to qualified maintenance links';
    END IF;
    ...
    IF observed_failure_events <> expected_failure_events
       OR observed_work_orders <> expected_work_orders THEN
        RAISE EXCEPTION 'Stage 13B failure-event/work-order grains do not reconcile: events %/%; work orders %/%', observed_failure_events, expected_failure_events, observed_work_orders, expected_work_orders;
    END IF;
    IF ABS(observed_downtime_hours - expected_downtime_hours) > 0.000001
       OR ABS(observed_repair_hours - expected_repair_hours) > 0.000001 THEN
        RAISE EXCEPTION 'Stage 13B event-level duration reconciliation failed; possible downtime multiplication';
    END IF;

    IF (SELECT COUNT(*) FROM analytics.vw_equipment_reliability_kpi) <> 255 THEN
        RAISE EXCEPTION 'Stage 13B reliability KPI view does not cover all 255 equipment rows';
    END IF;
    IF COALESCE((SELECT SUM(corrective_failure_count)
       ...
       OR COALESCE((SELECT SUM(corrective_work_order_count)
                    FROM analytics.vw_equipment_reliability_kpi), 0)
       <> expected_work_orders THEN
        RAISE EXCEPTION 'Stage 13B KPI output does not reconcile to canonical failure-event/work-order grains';
    END IF;
    IF ABS(COALESCE((SELECT SUM(linked_failure_downtime_hours)
       ...
       OR ABS(COALESCE((SELECT SUM(total_corrective_repair_hours)
        ...
        RAISE EXCEPTION 'Stage 13B KPI duration output indicates duplicated or omitted failure/work-order measures';
    END IF;
    IF expected_failure_events <> 46670 OR expected_work_orders <> 46670 THEN
        RAISE EXCEPTION 'Stage 13B accepted source snapshot changed from 46,670 failure events / work orders';
    END IF;
    ...
    WHERE mttr_hours <= 0 OR mean_calendar_inter_failure_hours <= 0
       ...
       OR reliability_availability_proxy > 1;
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'Stage 13B KPI sanity gate found % invalid equipment rows', bad_count;
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Stage 13B provenance gate expected only synthetic Velora integration data';
    END IF;
END
$validation$;
