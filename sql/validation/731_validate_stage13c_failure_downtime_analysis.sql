...
\echo ''
\echo '=== Stage 13C validation complete ==='

\echo '=== Mandatory Stage 13C gate ==='
DO $validation$
DECLARE
    bad_count bigint;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM analytics.vw_failure_reason_downtime_summary)
       OR NOT EXISTS (SELECT 1 FROM analytics.vw_equipment_failure_burden) THEN
        RAISE EXCEPTION 'Stage 13C accepted views are empty';
    END IF;
    ...
    IF COALESCE((SELECT SUM(downtime_event_count)
       ...
       <> expected_failure_events THEN
        RAISE EXCEPTION 'Stage 13C event populations do not reconcile at downtime/failure-event grain';
    END IF;
    IF ABS(COALESCE((SELECT SUM(corrective_linked_downtime_min)
                     FROM analytics.vw_failure_reason_downtime_summary), 0)
           - expected_corrective_downtime_min) > 0.000001 THEN
        RAISE EXCEPTION 'Stage 13C corrective downtime was multiplied or omitted';
    END IF;
    IF expected_downtime_events <> 170408
       OR expected_failure_events <> 46670 THEN
        RAISE EXCEPTION 'Stage 13C accepted source snapshot changed from 170,408 downtime / 46,670 corrective failure events';
    END IF;
    ...
    WHERE unplanned_downtime_share < 0 OR unplanned_downtime_share > 1
       ...
       OR cumulative_unplanned_downtime_share > 1.0000001;
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'Stage 13C Pareto gate found % invalid rows', bad_count;
    END IF;
    ...
    IF COALESCE((SELECT SUM(corrective_failure_count)
       ...
       OR ABS(COALESCE((SELECT SUM(linked_failure_downtime_hours)
        ...
        RAISE EXCEPTION 'Stage 13C equipment burden does not reconcile to canonical failure-event count and downtime';
    END IF;
END
$validation$;
